...
The following are the project's published reference benchmark, reproduced only when an operator supplies the verified case file (the harness records the fixture SHA-256 and Git commit).
...
Published reference: 4/5 correct, 30.75-day mean lead time on correct predictions, 11% false-positive rate on five controls.
...
Run `npm run backtest:full -- --file=<verified.json>` to measure current captured predictions.

### Prompt versioning

Every prompt change is gated by a regression suite. Promotion requires a two-proportion z-test at p < 0.05 against the labeled test-case database, with an atomic version swap. LangSmith tracks each version's evaluation runs.

IntentAnalyzer — 150 labeled test cases: 0.79 inference accuracy, 96% evidence citation rate, 97% schema pass on first attempt.
...
The production threshold is 0.88. `npm run dedup-calibration -- --file=<verified.json>` reports metrics for supplied labels but never edits the runtime threshold.

### RAG quality

The evaluator (`scripts/rag-eval.ts`) runs curated golden Q&A pairs through the real chat pipeline and scores faithfulness with an LLM judge (`min(correctness, groundedness)`, computed in code). It runs in CI on every push; a below-threshold result fails the build. The faithfulness floor is 0.75.
...
The adaptive cost router reduces this 15–30% under end-of-month budget pressure by downgrading eligible tasks to cheaper models.

---

## Latency Budget

Generated by `npm run latency-report`. Targets below (not yet measured):
...
Measured values replace targets as the system accumulates data in `agent_latencies`.
