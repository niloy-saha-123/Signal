...
const { cacheRedisMock, subscriberMock, duplicateMock, loggerMock } = vi.hoisted(() => {
...
import { publishSocketEvent, wireSocketRelay, type EmittableSocketServer } from "@/lib/socket-relay";
...
function getConnectionHandler(io: ReturnType<typeof createFakeIo>) {
  ...
  return call[1] as (socket: { on: ReturnType<typeof vi.fn>; join: ReturnType<typeof vi.fn>; leave: ReturnType<typeof vi.fn> }) => void;
}
...
describe("wireSocketRelay", () => {
  ...
  it("joins a socket to a competitor room on a valid competitor:join", () => {
    const io = createFakeIo();
    wireSocketRelay(io as unknown as EmittableSocketServer);
    const socket = { on: vi.fn(), join: vi.fn(), leave: vi.fn() };
  ...
  it("leaves a socket's competitor room on a valid competitor:leave", () => {
    const io = createFakeIo();
    wireSocketRelay(io as unknown as EmittableSocketServer);
    const socket = { on: vi.fn(), join: vi.fn(), leave: vi.fn() };
    getConnectionHandler(io)(socket);
