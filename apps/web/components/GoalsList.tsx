    ...
    <section className="rounded-lg bg-surface p-6">
        ...
        <ul className="mt-4 flex flex-col gap-2">
          {goals.map((goal) => (
            <li
              ...
              className="flex items-start gap-2 rounded-lg border border-line bg-surface-sunken px-3 py-2.5"
            >
              {editingId === goal.id ? (
                <div className="flex w-full items-center gap-2">
                  <input
                    ...
                    className="w-full rounded-lg border border-line bg-white px-3 py-1.5 text-sm text-ink focus:border-accent focus:outline-none"
                  />
                  <button
                    type="button"
          ...
          {goals.length === 0 && (
            <li className="rounded-lg border border-dashed border-line px-3 py-4 text-sm text-ink-secondary">
      ...
      <form onSubmit={handleAdd} className="mt-4 flex items-center gap-2">
        <input
          ...
          className="w-full rounded-lg border border-line bg-white px-3 py-2 text-sm text-ink placeholder:text-ink-secondary focus:border-accent focus:outline-none"
        />
        <button
          type="submit"
          disabled={adding || newGoal.trim().length === 0}
          className="shrink-0 rounded-lg bg-accent px-4 py-2 text-xs font-semibold text-white transition-opacity hover:opacity-90 disabled:opacity-30"
        >
          Add
        </button>
      </form>
